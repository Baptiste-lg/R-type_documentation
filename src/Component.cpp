...
void registerServerSystem(Registry& registry, sf::RenderWindow& window, AssetManager& textureManager, float &elapsed_time) {
    ...
    registry.add_system<DrawSystem, Drawable, Position, Depth>(std::make_unique<DrawSystem>(window, textureManager, registry));
    registry.add_system<FFTSystem, FFT>(std::make_unique<FFTSystem>(elapsed_time, registry));
    registry.add_system<ServerSystem, Network, State>(std::make_unique<ServerSystem>(registry, elapsed_time));
...
void registerClientSystem(Registry& registry, float &elapsed_time)
{
    std::cout << "deltaTime: " << elapsed_time << std::endl;
...
void cleanup_system(Registry& registry) {
    ...
    if (gameConfigComponents.empty()) {
        std::cerr << "GameConfig not set. Exiting cleanup_system." << std::endl;
        return;
    }
