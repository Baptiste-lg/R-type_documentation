...
class DrawSystem : public ISystem<Drawable, Position, Depth> {
private:
...
public:
    ...
    void sortEntities(sparse_array<Drawable>& drawables, sparse_array<Depth>& depths) {
        sortedEntities.clear();

        for (entity_t entity = 0; entity < drawables.size(); ++entity) {
            if (drawables.is_active(entity) && depths[entity] && drawables[entity]) {
        ...
        std::sort(sortedEntities.begin(), sortedEntities.end(), [&depths, &drawables](entity_t a, entity_t b) {
            if (depths[a]->z == depths[b]->z) {
                // Assuming Drawable has a textureName member
                return drawables[a]->textureName < drawables[b]->textureName;
            }
    ...
    void operator()(entity_t entity, sparse_array<Drawable>& drawables, sparse_array<Position>& positions, sparse_array<Depth>& depths) override {
        if (firstRun) {
            sortEntities(drawables, depths);
...
class ServerSystem : public ISystem<Network, State> {
private:
    float& deltaTime;
...
public:
    ServerSystem(Registry& registry, float& deltaTime) : registry(registry), deltaTime(deltaTime) {
        ...
        eventHandlerMap[PlayerJoined] = [this, &registry](const Event& event) {

            std::cout << "PlayerJoined event received" << std::endl;
            //we will add a new entity to the game using spawnedNonLocalPlayer
            entity_t new_entity = spawnNoneLocalPlayer(registry, {0, 0, 176, 96});
        ...
        eventHandlerMap[PlayerMoved] = [this, &registry](const Event& event) {
            // Your PlayerMoved event code here
    ...
    void operator()(entity_t entity, sparse_array<Network>& networks, sparse_array<State>& states) override {
        ...
        if (recvLen != -1) {
            ...
            if (client_id != -1) {
                ...
                std::string eventTypeStr = receivedMessage.substr(0, 2);  // Assuming 2-character identifiers
                std::string eventContent = receivedMessage.substr(3);  // The rest is content
                ...
                if (eventTypeIt != eventIdentifierMap.end()) {
                    ...
                    auto eventHandlerIt = eventHandlerMap.find(eventType);
                    if (eventHandlerIt != eventHandlerMap.end()) {
                        eventHandlerIt->second(newEvent);
                        newEvent.content = eventContent;
                        newEvent.clientID = client_id;
                        registry.eventBus.fireEvent(newEvent);
                    } else {
                        std::cerr << "Event handler not found for event type " << eventType << std::endl;
                    }
...
public:
    ...
    void listen_to_server() {
        ...
        while (true) {
            ...
            newEvent.type = parseEventType(buffer); // Function to parse buffer into EventType
            registry.eventBus.fireEvent(newEvent);
        }
    }
    ...
    void operator()(entity_t entity, sparse_array<State>& states) override {
        ...
        std::cout << "ClientSystem operator()" << std::endl;
        char input = getch();
